"""
FNO Feasibility - Option A: Physics Pseudo-Label
=================================================
Generates ground-truth from the deterministic physics formula
(parallel-plate + Sakurai-Tamaru fringe) so no SPEF matching is needed.

Compares: MLP vs Conv2D vs FNO-2D on the same dataset.
Output: convergence curves + final MAPE table.

Usage:
  python3 scripts/diag_fno_option_a.py --gpu 0 --n_tiles 2000 --epochs 80
"""

import sys, gzip, pickle, math, random, argparse, time
import numpy as np
import pandas as pd
...
# ─────────────────────────────────────────────
# Physics pseudo-label
# ─────────────────────────────────────────────

def compute_physics_cap(cuboids: np.ndarray) -> float:
    """
    Compute deterministic GND capacitance using parallel-plate + S-T fringe.
    Mimics flux_head.py but purely from geometry — no MLP modifier.
    Returns total estimated GND cap in fF.
    """
    target_mask = cuboids[:, 7] == 1.0
    target = cuboids[target_mask]
    if len(target) == 0:
        return 0.0

    total_cap = 0.0
    for cub in target:
        x, y, z, w, h, d, sem, is_tgt, eps, *_ = cub
        if z < 1e-3:
            continue
        area = w * h          # bottom face area μm²
        perimeter = 2 * (w + h)
        # Parallel plate to substrate
        c_plate = EPS_0 * eps * area / z
        # Sakurai-Tamaru fringe (sidewalls to substrate)
        c_fringe = EPS_0 * eps * perimeter * math.log1p(d / z) * (2.0 / math.pi)
        total_cap += c_plate + c_fringe

    return max(total_cap, 1e-6)
...
def flat_features(cuboids: np.ndarray) -> np.ndarray:
    ...
    return np.array([
        safe(tgt[:, 3], np.median),          # median wire width
        safe(tgt[:, 4], np.median),          # median wire height
        safe(tgt[:, 5], np.median),          # median thickness
        safe(tgt[:, 2], np.mean),            # mean z_abs
        safe(tgt[:, 8], np.mean, 1.0),       # mean eps
        math.log1p(len(tgt)),
        math.log1p(len(agg)),
        math.log1p(safe(area_arr, np.sum)),  # total area
        safe(tgt[:, 2], np.min),             # min z (nearest layer)
        safe(tgt[:, 3], np.max),             # max width
...
def build_dataset(n_tiles: int, grid_size: int, seed: int = 42):
    ...
    sample = df.sample(min(n_tiles * 3, len(df)), random_state=seed)
    ...
    F = torch.tensor(np.array(feats),  dtype=torch.float32)
...
def main():
    ...
    print(f"\n=== FNO Feasibility - Option A: Physics Pseudo-Labels ===")
    print(f"Device: {device} | Tiles: {args.n_tiles} | Grid: {args.grid_size}×{args.grid_size}")
    print(f"Epochs: {args.epochs} | LR: {args.lr}\n")

    # Build dataset
    print("Building dataset (physics pseudo-labels)...")
    ...
    print(f"Cap range: {torch.expm1(Y).min():.3f} – {torch.expm1(Y).max():.3f} fF\n")
    ...
    print(f"\n{'='*60}")
    print("OPTION A RESULT — Physics Pseudo-Label")
    print(f"{'='*60}")
    ...
    if fno_best < mlp_best * 0.80:
        verdict = "✓ SPATIAL FEATURES ARE STRONGLY USEFUL (+20%p gain)"
    elif fno_best < mlp_best * 0.90:
        verdict = "△ SPATIAL FEATURES MODERATELY USEFUL (+10%p gain)"
    else:
        verdict = "✗ SPATIAL FEATURES NOT USEFUL AT THIS RESOLUTION"
    print(f"\nVerdict: {verdict}")

    # Save summary
    out = Path('output_intel22') / 'diag_fno_option_a.txt'
    with open(out, 'w') as fp:
        fp.write(f"Option A — Physics Pseudo-Label\n")
        fp.write(f"Grid: {args.grid_size}×{args.grid_size}, Tiles: {N}, Epochs: {args.epochs}\n\n")
        for name in results:
