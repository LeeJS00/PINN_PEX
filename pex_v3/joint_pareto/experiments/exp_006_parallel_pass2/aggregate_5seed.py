...
Reads `seed{0..4}_metrics.json` and `seed{0..4}_runtime.json` from --in-dir
and emits a single measurement.json matching the schema consumed by
`pex_v3/joint_pareto/scripts/admit_to_frontier.py` and the leaderboard.

Required output fields (admit_to_frontier expects these):
  - n_seeds
  - wall_clock_s              (mean across seeds)
  - mean_of_mean_mape         (per-net total cap MAPE mean)
  - mean_of_median_mape
  - mean_of_p95_mape
  - gnd_matched_mean
  - cpl_matched_mean
...
def main() -> int:
    ...
    out = {
        ...
        "variant_id": "v7_parallel",
        "label": "Path-2 v7 parallel pass-2 (16 workers)",
