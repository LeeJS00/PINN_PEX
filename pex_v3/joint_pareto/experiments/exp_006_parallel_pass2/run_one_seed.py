#!/usr/bin/env python3
"""run_one_seed.py — single-seed driver for exp_006 parallel pass-2.

Pipeline (matches `pex_v3/scripts/40_*` + `41_*` chain):
    1. Build autonomous_fast SPEF using the parallel engine.
...
Outputs:
    <out_dir>/seed{N}_autonomous_fast.spef
    <out_dir>/seed{N}_xgb.spef
    <out_dir>/seed{N}_HERO.spef
    <out_dir>/seed{N}_runtime.json   (per-stage breakdown of ENGINE only)
    <out_dir>/seed{N}_metrics.json   (per-channel matched/unmatched MAPE, R²)
    <out_dir>/seed{N}_compare/spef_comparison_report.csv
"""
...
# This file lives at pex_v3/joint_pareto/experiments/exp_006_parallel_pass2/
# parents[4] = PINNPEX repo root, where `src/` and `configs/` live.
...
from engine import write_fast_autonomous_spef_parallel  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--seed", type=int, required=True, help="XGB seed (0..4)")
    ...
    p.add_argument("--skip-engine", action="store_true",
                   help="Reuse a previously generated autonomous_fast SPEF (debug)")
    return p.parse_args()
...
def _per_channel_metrics(compare_csv: Path, xgb_csv: Path) -> dict:
    """Compute mean/median/p95 of per-net total MAPE, plus per-channel matched/unmatched MAPE."""
...
def main() -> int:
    ...
    auto_spef = args.out_dir / f"seed{seed}_autonomous_fast.spef"
    ...
    if not args.skip_engine:
        ...
        print(f"  (skip-engine) reusing {auto_spef}")
