...
Strike #8: extend self_features 16 → 29 by adding sister's v6 cell-internal
...
- features-csv: all_designs_with_pincap.csv (62 cols, +7 Liberty pin-cap features)
...
_SELF_FEATURE_COLS_EXT = [
    ...
    "fanout",
    "aspect_ratio",
    # ---- new 7 Liberty pin-cap features (PDK lib) ----
    "pin_cap_total",
    "pin_cap_input_total",
    "pin_cap_max",
    "pin_cap_mean",
...
def parse_args():
    ...
    p.add_argument(
        ...
        default=_PROJECT_ROOT / "pex_v3" / "output" / "phase1_mesh_with_pincap_smoke",
    )
...
def main():
    ...
    print(f">>> Strike #8 mesh + Liberty pin caps — seed {args.seed}")
    print(f">>> features:  {args.features_csv}")
    print(f">>> cuboids:   {args.cuboid_dir}")
    print(f">>> output:    {args.output_dir}")
    print(f">>> device:    {args.device}")
    print(f">>> self_dim:  {SELF_DIM_EXT} (was 16, +7 Liberty pin-cap features)")
    ...
    snap["task"] = "phase1_mesh_with_pincap_smoke"
