...
Strike #8b (counts only): extend self_features 16 → 29 by adding sister's v6 cell-internal
...
- features-csv: all_designs_with_pincap.csv (62 cols, +3 Liberty pin counts only features)
...
_SELF_FEATURE_COLS_EXT = [
    ...
    "fanout",
    "aspect_ratio",
    # ---- 3 Liberty pin counts ONLY (design-invariant, no fF values) ----
    "n_pins_lib_matched",
    "n_input_pins_lib",
    "n_output_pins_lib",
]
SELF_DIM_EXT = len(_SELF_FEATURE_COLS_EXT)  # 19
...
def parse_args():
    ...
    p.add_argument(
        ...
        default=_PROJECT_ROOT / "pex_v3" / "output" / "phase1_mesh_pincap_counts_smoke",
    )
...
def main():
    ...
    print(f">>> Strike #8b (counts only) mesh + Liberty pin caps — seed {args.seed}")
    print(f">>> features:  {args.features_csv}")
    print(f">>> cuboids:   {args.cuboid_dir}")
    print(f">>> output:    {args.output_dir}")
    print(f">>> device:    {args.device}")
    print(f">>> self_dim:  {SELF_DIM_EXT} (was 16, +3 Liberty pin counts only features)")
    ...
    snap["task"] = "phase1_mesh_pincap_counts_smoke"
    ...
    print()
    print("=" * 60)
    print(f">>> LIBERTY COUNTS ONLY STRIKE RESULT")
