#!/usr/bin/env python3
"""
37_finetune_mesh_clamp_norm_smoke.py — ClampNorm single-seed smoke.

Variant: HybridPexV3MeshClampNorm — drop-in replacement for
HybridPexV3Mesh that swaps the element-wise clamp on the residual
logit for a joint per-net (gnd, cpl) L2 norm-projection clamp.

Hypothesis (full doc:
pex_v3/experiments/auto_optimize_2026_05_03/variants/clamp_norm/DESIGN.md):
    Element-wise hard clamp at C produces a 0/1 gradient cliff that
    causes encoder weights to oscillate when |δ| is near the cap. In
    Phase 2 (C = log 4), this manifests as ±5 pp valid-MAPE swings.
    Joint norm-projection has continuous Jacobian at the boundary →
    smoother optimization → tighter Phase 2 stability.
...
Output:
  pex_v3/experiments/auto_optimize_2026_05_03/outputs/clamp_norm/seed42/
...
from src.models.hybrid_v3_mesh_clamp_norm import HybridPexV3MeshClampNorm  # noqa: E402
...
def parse_args():
    p = argparse.ArgumentParser(description="ClampNorm smoke (single seed)")
    p.add_argument(
    ...
    p.add_argument(
        ...
        / "auto_optimize_2026_05_03" / "outputs" / "clamp_norm" / "seed42",
    )
    ...
    p.add_argument("--gpu", type=int, default=7,
                   help="CUDA device index (default 7 per PLAN.md GPU allocation)")
...
# ---------------------------------------------------------------------------
# Joint-clamp evaluator (ClampNorm-aware)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# eval_logger compatibility wrapper — install joint-clamp predict_* shims so
# that `collect_per_net_predictions` (which calls predict_gnd / predict_cpl
# back-to-back) sees the SAME logits a joint forward would see.
# ---------------------------------------------------------------------------

class _JointShim(torch.nn.Module):
    """Wraps HybridPexV3MeshClampNorm so predict_gnd / predict_cpl return
    joint-clamp results. Used only for eval_logger parquet generation.
    """
    def __init__(self, model: HybridPexV3MeshClampNorm):
        super().__init__()
        self.model = model

    def predict_gnd(self, ag, sf, cb, mk):
        # Need pair_features + analytic_cpl; the eval_logger does not
        # provide them to predict_gnd. We fall back to the standalone
        # API on the underlying model, which assumes logit_cpl=0.
        # For the 5-seed lock + paper, prefer the joint evaluator above.
        return self.model.predict_gnd(ag, sf, cb, mk)

    def predict_cpl(self, ac, pf, cb, mk):
        return self.model.predict_cpl(ac, pf, cb, mk)
...
def main() -> None:
    ...
    print(f">>> ClampNorm smoke — seed {args.seed}")
    ...
    snap["task"] = "clamp_norm_smoke"
    snap["calibration"] = cal_summary["calibration"]
    snap["n_epochs"] = args.n_epochs
    snap["model_params"] = pc["total"]
    snap["model_class"] = "src.models.hybrid_v3_mesh_clamp_norm.HybridPexV3MeshClampNorm"
    snap["seed"] = args.seed
    ...
    for epoch in range(args.n_epochs):
        ...
        for batch in train_loader:
            ...
            # JOINT forward (the clamp-norm path)
    ...
    # 8. Decision gate
    BASELINE_TEST = {"gnd": 0.2049, "cpl": 0.1553, "total": 0.0827}
    THRESH        = {"gnd": 0.195,  "cpl": 0.145,  "total": 0.0727}
    REG_TOL       = 0.005
    ...
    pass_any = (gnd_t <= THRESH["gnd"]) or (cpl_t <= THRESH["cpl"]) or (tot_t <= THRESH["total"])
    no_regression = (
        (gnd_t - BASELINE_TEST["gnd"])   <= REG_TOL and
        (cpl_t - BASELINE_TEST["cpl"])   <= REG_TOL and
        (tot_t - BASELINE_TEST["total"]) <= REG_TOL
    )

    # Curriculum-transition gain check
    ...
    verdict = "PASS" if (pass_any and no_regression and transitions_preserved) else "FAIL"
    print()
    print("=" * 70)
    print(f">>> ClampNorm smoke verdict: {verdict}")
    print(f"    - test gnd:   {gnd_t*100:6.3f}%  (thresh ≤ {THRESH['gnd']*100:.2f}%)  "
          f"baseline {BASELINE_TEST['gnd']*100:.2f}%")
    print(f"    - test cpl:   {cpl_t*100:6.3f}%  (thresh ≤ {THRESH['cpl']*100:.2f}%)  "
          f"baseline {BASELINE_TEST['cpl']*100:.2f}%")
    print(f"    - test total: {tot_t*100:6.3f}%  (thresh ≤ {THRESH['total']*100:.2f}%)  "
          f"baseline {BASELINE_TEST['total']*100:.2f}%")
    ...
    with open(args.output_dir / "summary.json", "w") as f:
        json.dump({
            "seed": args.seed,
            "variant": "HybridPexV3MeshClampNorm",
    ...
    # eval_logger (uses the standalone-API shims; logit_cpl=0 fallback)
    # Acceptable for parquet artifact since aggregator + stratifier read
    # per-net medians; the joint evaluator above is the source of truth
    # for the headline summary.json metrics.
