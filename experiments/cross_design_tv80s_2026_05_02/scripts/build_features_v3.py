"""Build v3 per-net feature parquets (multi-radius densities)."""
...
from src.feat_extract_v3 import extract_features_for_net_v3, FEATURE_NAMES_V3
...
def _proc_one(args):
    pkls, name = args
    try:
        f = extract_features_for_net_v3([Path(p) for p in pkls], cutoff_um=cfg.CPL_CUTOFF_UM)
    except Exception:
        traceback.print_exc()
        return None
    if f is None:
...
def build_v3(design: str, manifest: pd.DataFrame, force: bool, n_workers: int):
    out_dir = cfg.CACHE_DIR / "features_v3"
...
def main():
    ...
    for d in designs:
        try:
            build_v3(d, manifest, args.force, args.workers)
        except Exception:
            traceback.print_exc()
        gc.collect()
