"""Train direct c_gnd regressor: 5 LGBM + 5 CatBoost.

Replaces (or blends with) the indirect total × ratio split.
Trained on 9 designs, validated on nova, evaluated on tv80s.
Output: output/spef_e2e/cgnd_direct/{lgbm_seed{0..4}.pkl, cat_seed{0..4}.cbm, fcols.json}
"""
...
def main():
    ...
    eps = 1e-4
    y_tr = np.log(train["c_gnd_fF"].to_numpy().clip(min=eps))
    y_va = np.log(val["c_gnd_fF"].to_numpy().clip(min=eps))
    y_te_lin = test["c_gnd_fF"].to_numpy()

    X_tr = train[fcols].to_numpy(np.float32)
    X_va = val[fcols].to_numpy(np.float32)
    X_te = test[fcols].to_numpy(np.float32)

    out_dir = cfg.OUTPUT_DIR / "spef_e2e" / "cgnd_direct"
    ...
    print("\n=== Ensemble (uniform mean) ===", flush=True)
    all_preds = []
