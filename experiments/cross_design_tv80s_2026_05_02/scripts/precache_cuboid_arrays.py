"""
Pre-cache cuboid arrays per (design, net) into a single npz per design.

Per-net storage:
    target:  (T_max=128, 10)  padded
    agg:     (A_max=256, 10)  padded
    pwr:     (P_max=128, 10)  padded
    n_t, n_a, n_p: actual counts (capped at maxes)
    abs_geom for target only (for distance scaling normalisation later if needed)

Output: cache/cuboid_arr/<design>.npz
        with arrays:
            target_arr: (N, T_max, 10)
            agg_arr:    (N, A_max, 10)
            pwr_arr:    (N, P_max, 10)
            n_t, n_a, n_p: (N,) int
            net_names:  (N,) string
"""
from __future__ import annotations

import argparse
import gzip
import multiprocessing as mp
import pickle
import sys
import time
from pathlib import Path

import numpy as np
import pandas as pd

_HERE = Path(__file__).resolve().parent
_WS = _HERE.parent
sys.path.insert(0, str(_WS))

from configs import cfg


T_MAX = 128
A_MAX = 256
P_MAX = 128


def _hash_geom(arr: np.ndarray) -> np.ndarray:
    rounded = np.round(arr, 4).astype(np.float32)
    rb = rounded.tobytes()
    rec_len = arr.shape[1] * 4
    out = np.empty(len(arr), dtype=np.int64)
    for i in range(len(arr)):
        out[i] = hash(rb[i * rec_len:(i + 1) * rec_len])
    return out


def _truncate_priority(c_all: np.ndarray, ag_all: np.ndarray, target_xy: np.ndarray, k: int):
    """Sort cuboids by distance to target center, keep top-k."""
    if c_all.shape[0] == 0:
        return c_all, 0
    if c_all.shape[0] <= k:
        return c_all, c_all.shape[0]
    # priority: distance to target xy center
    if target_xy.shape[0] > 0:
        center_x = target_xy[:, 0].mean()
        center_y = target_xy[:, 1].mean()
        d2 = (ag_all[:, 0] - center_x) ** 2 + (ag_all[:, 1] - center_y) ** 2
        idx = np.argsort(d2)[:k]
    else:
        idx = np.arange(k)
    return c_all[idx], k


def _process_net(args):
    pkl_paths_str, name = args
    paths = [Path(p) for p in pkl_paths_str]
    records = []
    for p in paths:
        try:
            with gzip.open(p, "rb") as fh:
                records.append(pickle.load(fh))
        except Exception:
            continue
    if not records:
        return None

    target_rows = []; agg_rows = []; pwr_rows = []
    target_geom = []; agg_geom = []; pwr_geom = []
    for rec in records:
        c = rec["cuboids"]; ag = rec["abs_geometries"]
        m_t = c[:, 7] == 1.0
        m_pwr = (c[:, 7] == 0.0) & (c[:, 9] >= 0.6)
        m_agg = (c[:, 7] == 0.0) & (c[:, 9] < 0.6)
        target_rows.append(c[m_t]); target_geom.append(ag[m_t])
        agg_rows.append(c[m_agg]); agg_geom.append(ag[m_agg])
        pwr_rows.append(c[m_pwr]); pwr_geom.append(ag[m_pwr])

    if not target_rows or all(r.shape[0] == 0 for r in target_rows):
        return None

    T = np.concatenate(target_rows, axis=0).astype(np.float32)
    Tg = np.concatenate(target_geom, axis=0).astype(np.float32)
    A = np.concatenate(agg_rows, axis=0).astype(np.float32) if agg_rows else np.zeros((0, 10), dtype=np.float32)
    Ag = np.concatenate(agg_geom, axis=0).astype(np.float32) if agg_geom else np.zeros((0, 6), dtype=np.float32)
    P = np.concatenate(pwr_rows, axis=0).astype(np.float32) if pwr_rows else np.zeros((0, 10), dtype=np.float32)
    Pg = np.concatenate(pwr_geom, axis=0).astype(np.float32) if pwr_geom else np.zeros((0, 6), dtype=np.float32)

    # dedupe target by hash
    if T.shape[0] > 0:
        h = _hash_geom(Tg)
        _, idx = np.unique(h, return_index=True)
        idx = np.sort(idx)
        T = T[idx]; Tg = Tg[idx]

    # truncate (priority by distance to target center)
    T_keep, n_t = _truncate_priority(T, Tg, Tg, T_MAX)
    A_keep, n_a = _truncate_priority(A, Ag, Tg, A_MAX)
    P_keep, n_p = _truncate_priority(P, Pg, Tg, P_MAX)

    # pad to fixed sizes
    T_pad = np.zeros((T_MAX, 10), dtype=np.float32)
    A_pad = np.zeros((A_MAX, 10), dtype=np.float32)
    P_pad = np.zeros((P_MAX, 10), dtype=np.float32)
    T_pad[:n_t] = T_keep
    A_pad[:n_a] = A_keep
    P_pad[:n_p] = P_keep

    return name, T_pad, A_pad, P_pad, n_t, n_a, n_p


def build_design(design: str, manifest: pd.DataFrame, force: bool, n_workers: int):
    out_dir = cfg.CACHE_DIR / "cuboid_arr"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"{design}.npz"
    if out_path.exists() and not force:
        print(f"[{design}] cached → skip"); return out_path

    sub = manifest[manifest["design_name"] == design]
    if sub.empty: return None
    sub = sub.copy()
    sub["abs_path"] = str(cfg.DATA_ROOT) + "/" + sub["rel_path"].astype(str)
    grouped = sub.groupby("net_name")["abs_path"].apply(list)
    work = [(grouped[n], n) for n in grouped.index]

    rows = []
    t0 = time.time()
    with mp.Pool(processes=n_workers) as pool:
        for i, res in enumerate(pool.imap_unordered(_process_net, work, chunksize=4)):
            if res is None: continue
            rows.append(res)
            if (i + 1) % 2000 == 0:
                rate = (i + 1) / (time.time() - t0)
                eta = (len(work) - i - 1) / max(rate, 1e-3)
                print(f"[{design}]  {i+1}/{len(work)}  {rate:.1f}/s eta {eta:.0f}s", flush=True)

    if not rows:
        print(f"[{design}] no rows"); return None

    rows.sort(key=lambda r: r[0])
    net_names = np.array([r[0] for r in rows])
    T_arr = np.stack([r[1] for r in rows], axis=0)
    A_arr = np.stack([r[2] for r in rows], axis=0)
    P_arr = np.stack([r[3] for r in rows], axis=0)
    n_t = np.array([r[4] for r in rows], dtype=np.int32)
    n_a = np.array([r[5] for r in rows], dtype=np.int32)
    n_p = np.array([r[6] for r in rows], dtype=np.int32)

    np.savez_compressed(
        out_path,
        target=T_arr, aggressor=A_arr, power=P_arr,
        n_target=n_t, n_agg=n_a, n_pwr=n_p,
        net_names=net_names,
    )
    print(f"[{design}] → {out_path}  {len(rows)} nets  in {time.time()-t0:.1f}s, file={out_path.stat().st_size/1e6:.1f} MB")
    return out_path


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--designs", nargs="+", default=None)
    ap.add_argument("--workers", type=int, default=12)
    ap.add_argument("--force", action="store_true")
    args = ap.parse_args()
    designs = args.designs or cfg.ALL_DESIGNS

    print(f"loading manifest: {cfg.MANIFEST_PATH}")
    manifest = pd.read_csv(cfg.MANIFEST_PATH)
    for d in designs:
        try:
            build_design(d, manifest, args.force, args.workers)
        except Exception:
            import traceback; traceback.print_exc()


if __name__ == "__main__":
    main()
