"""Phase 12c — fit with v6 features = v4 + cell SIZE + signal-OBS (power filtered)."""
...
def main():
    ...
    abls = [
        ...
        ("+ v6 signal OBS",
            ["nsq_M", "rsq_M", "nvian_", "one", "n_pin_PIN", "n_pin_inst", "pin_nsq_M",
             "v6_obs_signal_nsq_M", "v6_obs_n_via_v"]),
        ("+ v6 signal OBS + cell SIZE",
            ["nsq_M", "rsq_M", "nvian_", "one", "n_pin_PIN", "n_pin_inst", "pin_nsq_M",
             "v6_obs_signal_nsq_M", "v6_obs_n_via_v", "v6_cell_"]),
        ("+ all v6",
            ["nsq_M", "rsq_M", "nvian_", "one", "n_pin_PIN", "n_pin_inst", "pin_nsq_M",
             "v6_"]),
    ]
    ...
    # Stage 2 with all v6 features + 1-hot
    ...
    print(f"\nStage 2 fcols (with v6 + 1-hot): {len(fcols_full) + n_des}", flush=True)
    ...
    print(f"\n=== v6 Stage 2 ensemble ===", flush=True)
    ...
    print(f"\nLength-stratified (v6 ensemble):", flush=True)
    ...
    test_df["R_pred_v6"] = pred_s2_test
    test_df["ape_v6"] = ape
    test_df.to_parquet(_V3 / "outputs" / "test_predictions_v6.parquet")
    out = {"stage1_ablations": [{k: v for k, v in r.items() if k != "coef"} for r in rows_s1],
           "stage1_best_label": best["label"],
           "stage1_best_test":  best["test_mape"],
           "stage2_test_MAPE":  float(np.mean(ape)),
           "stage2_median":     float(np.median(ape)),
           "stage2_p90":        float(np.percentile(ape, 90)),
           "stage2_config":     cfg,
           "n_seeds":           n_seeds}
    with open(_V3 / "outputs" / "v6_summary.json", "w") as f:
