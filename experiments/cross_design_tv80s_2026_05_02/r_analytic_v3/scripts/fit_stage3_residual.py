"""Phase 10 — Stage 3 stacking on Stage 2 ensemble residuals.

Stage 2 ensemble (5-seed cfg1) gave 2.46% test MAPE. We now train a Stage 3
LGBM on its residuals to capture remaining patterns. This is "boosting on
boosting" — diminishing returns expected, but worth trying.
"""
...
def main():
    ...
    # ---------------- Stage 2: 5-seed ensemble (relative residual) ----------------
    ...
    for seed in range(n_seeds):
        ...
        z2_test_seeds.append(gbm.predict(Xs_full))
        z2_train_seeds.append(gbm.predict(Xt_full))
        ts = pred_lin_test * (1 + z2_test_seeds[-1])
        print(f"  S2 seed {seed}: test MAPE = {np.mean(np.abs(ts-ys)/ys)*100:.4f}%", flush=True)

    z2_test_mean = np.mean(z2_test_seeds, axis=0)
    z2_train_mean = np.mean(z2_train_seeds, axis=0)
    pred_s2_train = pred_lin_train * (1 + z2_train_mean)
    pred_s2_test  = pred_lin_test  * (1 + z2_test_mean)
    print(f"\n=== Stage 2 ensemble ===", flush=True)
    _stats("S2 train", pred_s2_train, yt)
    _stats("S2 test",  pred_s2_test,  ys)

    # ---------------- Stage 3: GBT on Stage 2 residuals ----------------
    z3_train = (yt - pred_s2_train) / np.maximum(pred_s2_train, 1e-3)
    print(f"\nStage 3: GBT on relative residual of Stage 2 (no per-design 1-hot here)", flush=True)
    # Use the same physical features WITHOUT the per-design 1-hot (already absorbed)
    fcols_s3 = _select(train_dfs + [test_df], pref_full[:-2])
    Xt_s3 = np.vstack([_design_matrix(d, fcols_s3) for d in train_dfs])
    Xs_s3 = _design_matrix(test_df, fcols_s3)
    # add Stage 2 prediction features
    ...
    Xs_s3 = np.column_stack([Xs_s3, pred_lin_test,  pred_s2_test,
                              np.log(np.maximum(pred_s2_test, 1e-3)),
                              np.sqrt(np.maximum(pred_s2_test, 1e-3))])
    ...
    print(f"\nLength-stratified (Stage 3 stacked):", flush=True)
    ...
    test_df["R_pred_stage3"] = pred_final_test
    test_df["ape_stage3"] = ape
    test_df.to_parquet(_V3 / "outputs" / "test_predictions_stage3.parquet")
    out = {"final_test_MAPE":      float(np.mean(ape)),
           ...
           "stage2_config":        cfg2,
           "stage3_config":        cfg3,
           "stage3_features":      fcols_s3 + ["pred_lin", "pred_s2", "log_pred_s2", "sqrt_pred_s2"],
           "n_seeds_s2":           n_seeds,
           "n_seeds_s3":           n_seeds3}
    with open(_V3 / "outputs" / "stage3_summary.json", "w") as f:
